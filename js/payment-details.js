...
async function handlePaymentSubmit(e) {
    ...
    try {
        ...
        const purchaseToSave = {
            ...
            eventDate: purchaseData.eventDate, // Ya es ISO string
            eventLocation: purchaseData.eventLocation,
            ticketPrice: purchaseData.ticketPrice,
            // Almacenar en Firestore con el formato "seat-A1" para consistencia con la subcolección de asientos
            ...
            paymentDetails: { // No guardar CVV real, solo fines demostrativos
        ...
        // 2. Marcar asientos como comprados en la subcolección 'seats' del evento
        const eventSeatsCollectionRef = collection(db, 'eventos', purchaseData.eventId, 'seats');
        const batch = writeBatch(db); // Usar un batch para operaciones atómicas

        for (const seatId of purchaseData.selectedSeats) {
            const seatDocRef = doc(eventSeatsCollectionRef, `seat-${seatId}`); // Añadir prefijo para Firestore
            batch.set(seatDocRef, { // Usar set con merge para actualizar o crear si no existe
                status: 'occupied', // Cambiado a 'occupied' para ser más consistente
                userId: currentUser.uid,
