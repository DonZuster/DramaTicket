...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Esto es vital para habilitar/deshabilitar la selección de asientos y el botón de pago.
    onAuthStateChanged(auth, (user) => {
        currentUser = user;
        updateUIBasedOnAuth(); // Actualizar el botón de continuar y mensajes
        // Volver a renderizar los asientos para habilitar/deshabilitar selección si el estado de auth cambia
        if (currentEvent) {
...
async function loadEventDetailsFromFirestore(eventId) {
    ...
    try {
        ...
        if (eventDocSnap.exists()) {
            ...
            updateEventSummaryUI();
            renderSeats(currentEvent.id); // Renderizar asientos una vez cargado el evento
        } else {
...
async function renderSeats(currentEventId) {
    ...
    onSnapshot(seatsCollectionRef, (snapshot) => {
        ...
        snapshot.forEach(doc => {
            ...
            if (seatData.status === 'booked' || seatData.status === 'purchased') {
