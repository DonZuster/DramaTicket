...
async function loadUserProfile(user) {
    try {
        ...
        if (userDocSnap.exists()) {
            ...
            // Resolver fecha de membresía desde distintos campos
            let memberSinceDate = null;

            if (currentUserData.createdAt) {
                memberSinceDate = currentUserData.createdAt.toDate
                    ? currentUserData.createdAt.toDate()
                    : currentUserData.createdAt;
            } else if (currentUserData.registeredAt) {
                memberSinceDate = currentUserData.registeredAt.toDate
                    ? currentUserData.registeredAt.toDate()
                    : currentUserData.registeredAt;
            } else if (user.metadata && user.metadata.creationTime) {
                memberSinceDate = new Date(user.metadata.creationTime);
            }

            if (userRegisteredDateSpan) {
                userRegisteredDateSpan.textContent = memberSinceDate
                    ? formatTimestamp(memberSinceDate)
                    : 'N/A';
            }
        }
